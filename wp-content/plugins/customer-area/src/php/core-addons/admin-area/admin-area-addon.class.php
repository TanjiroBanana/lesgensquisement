...
class CUAR_AdminAreaAddOn extends CUAR_AddOn {
	...
	public function run_addon( $plugin ) {
		...
		if ( is_admin() ) {
			...
			// Settings
			add_action( 'cuar/core/settings/print-settings?tab=cuar_core', array(
				&$this,
				'print_core_settings'
			), 20, 2 );
			add_filter( 'cuar/core/settings/validate-settings?tab=cuar_core', array(
				&$this,
	...
	public function is_admin_area_access_restricted() {
		return true == $this->plugin->get_option( self::$OPTION_RESTRICT_ADMIN_AREA_ACCESS );
	}
			...
			$capability_groups['cuar_general']['groups'] = array(
				'back-office'  => array(
					...
					'capabilities' => array(
						...
						'view-customer-area-menu' => __( 'View the WP Customer Area menu', 'cuar' ),
					)
				),
				'front-office' => array(
	...
	public function plugin_version_upgrade( $from_version, $to_version ) {
		...
		// Need to migrate dates from Content Expiry add-on if main plugin is before 8.2.0
		if (version_compare($from_version, '8.1.98', '<'))
		{
			$this->migrate_expiry_dates();
		}
	}

	/**
	 * Function used to update Content Expiry post_meta fields while updating to 8.2.X version of the main plugin
	 *
	 * @return void
	 */
	protected function migrate_expiry_dates()
	{
		global $wpdb;

		$meta_cuar_valid_from = 'cuar_valid_from';
		$meta_cuar_valid_until = 'cuar_valid_until';

		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}postmeta WHERE meta_key = %s OR meta_key = %s",
				$meta_cuar_valid_from,
				$meta_cuar_valid_until
			)
		);
		...
		foreach ($rows as $row)
		{
			...
			{
				$posts[$row->post_id] = [null, null];
			}

			if ($row->meta_key === $meta_cuar_valid_from && $row->meta_value !== null)
			{
				$posts[ $row->post_id ][0] = $row->meta_value;
			}

			if ($row->meta_key === $meta_cuar_valid_until && $row->meta_value !== null)
			{
				$posts[ $row->post_id ][1] = $row->meta_value;
			}
		}

		foreach ($posts as $post_id => $dates)
		{
			...
			$this->migrate_expiry_dates_update_validity($post_id, $dates[0], $dates[1]);
		}
	}

	/**
	 * This function is basically a copy of update_validity from Content Expiry, which should be used to update dates
	 * from posts using Content Expiry data, when the main plugin is updated.
	 * Copied the function to make sure the data is updated on main plugin update, even if the add-on is unactive
	 *
	 * @param $post_id
	 * @param $valid_from
	 * @param $valid_until
	 ...
	 * @return void
	 */
	public function migrate_expiry_dates_update_validity($post_id, $valid_from, $valid_until)
	{
		$meta_cuar_valid_from = 'cuar_valid_from';
		$meta_cuar_valid_until = 'cuar_valid_until';
		$meta_cuar_valid_from_default = '2000-01-01';
		$meta_cuar_valid_until_default = '2200-12-31';

		if (!empty($valid_from) && !empty($valid_until))
		{
			$from = DateTime::createFromFormat('Y-m-d', $valid_from);
			$until = DateTime::createFromFormat('Y-m-d', $valid_until);
		...
		if (!empty($valid_from))
		{
			update_post_meta($post_id, $meta_cuar_valid_from, $valid_from);
		}
		else
		{
			update_post_meta($post_id, $meta_cuar_valid_from, $meta_cuar_valid_from_default);
		}

		if (!empty($valid_until))
		{
			update_post_meta($post_id, $meta_cuar_valid_until, $valid_until);
		}
		else
		{
			update_post_meta($post_id, $meta_cuar_valid_until, $meta_cuar_valid_until_default);
		}
	}
